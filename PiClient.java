...
public class PiClient extends JFrame implements MouseListener,MouseMotionListener,KeyListener,GpioPinListenerDigital {
	...
	private int current_x=3,current_y=3;

	private final GpioController gpio=GpioFactory.getInstance();
	private final GpioPinDigitalInput swt1 = gpio.provisionDigitalInputPin(RaspiPin.GPIO_01, PinPullResistance.PULL_DOWN);
	private final GpioPinDigitalInput swt2 = gpio.provisionDigitalInputPin(RaspiPin.GPIO_02, PinPullResistance.PULL_DOWN);
	...
	public PiClient() {
		...
		setTitle("PiClient");//ウィンドウのタイトルを設定する
		...
		for(int i=0;i<8;i++){
			for(int j=0;j<8;j++){
				...
				buttonArray[i][j].setBorder(new LineBorder(Color.BLUE,4,true));
				buttonArray[i][j].setBorderPainted(false);
			}
		}

		buttonArray[0][0].addKeyListener((KeyListener)this);
		buttonArray[current_y][current_x].setBorderPainted(true);
		...
		passbutton.addKeyListener(this);
		...
		swt1.addListener((GpioPinListenerDigital)this);
		swt2.addListener((GpioPinListenerDigital)this);
	}
	...
	public void handleGpioPinDigitalStateChangeEvent(GpioPinDigitalStateChangeEvent e){
		if(e.getEdge()!=PinEdge.FALLING) return;
		System.out.println("スイッチ入力");
		if(e.getPin()==swt1){
			doSpace();
		}else if(e.getPin()==swt2){
			doEnter();
		}
		repaint();
	}
	...
	public void doEnter(){
		...
		int theArrayIndex=current_y+8*current_x;
		JButton theButton=buttonArray[current_y][current_x];
		if(myColor==myTurn){
			
			if(judgeButton(theArrayIndex)){
				if(myColor==0){
					theButton.setIcon(blackIcon);
				}
				else{
	...
	public void doLeft(){
		if(current_y>0){
			buttonArray[current_y][current_x].setBorderPainted(false);
			current_y--;
			buttonArray[current_y][current_x].setBorderPainted(true);
		}
	}
	...
	public void start(){
		try{
			...
			GpioPinListenerAnalog listener = new GpioPinListenerAnalog()
			{
				...
				String gName = event.getPin().getName();

				if(gName.equals("MyAnalogInput-CH0") && value == 0){
					doEnter();
				}else if(gName.equals("MyAnalogInput-CH1") && value < 300){
					doLeft();
				}else if(gName.equals("MyAnalogInput-CH1") && value > 800){
					doRight();
				}else if(gName.equals("MyAnalogInput-CH2") && value < 300){
					doUp();
				}else if(gName.equals("MyAnalogInput-CH2") && value > 800){
					doDown();
				}
